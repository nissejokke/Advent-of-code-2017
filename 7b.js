...
let searchChildren = (needle, haystack) => {
    let keys = Object.keys(haystack);
    let matches = keys.filter(k => k === needle);
    if (matches.length > 0)
        return haystack[matches[0]];
    
    for (var i = 0; i < keys.length; i++) {
        let found = searchChildren(needle, haystack[keys[i]]);
        if (found)
            return found;
    }
}
...
input.split(/\n/)
    .map(row => {
        let inp = row.split(/ \(\d+\)/);
        let parent = inp[0];
        let children = inp[1].substr(4);

        let m = row.match(/\((\d+)\)/);
        let weight = parseInt(m[1]);
        ...
        if (children)
            children.split(/\s*,\s*/)
                .forEach(c => map[parent][c] = {})
    })

Object.keys(map)
    .filter(key => {
        let haystack = map[key];
        delete map[key];
        let found = searchChildren(key, map);
        if (!found) {
            map[key] = haystack;
        }
        else {
            Object.keys(haystack)
                .forEach(k => found[k] = haystack[k])
        }
            
    });

let sum = (leaf, total, depth, parentKey) => {
    let sums = [];
    Object.keys(leaf)
        .forEach(key => {
            if (isNaN(weights[key]) || key === "eqgvf")
                throw new Error('NaN');
            
            let towerWeight = sum(leaf[key], 0, depth + 1, key);
            let discWeight = weights[key];
            let added = discWeight + towerWeight;

            if (parentKey === 'exrud' || parentKey === 'rbzmniw' || parentKey === 'gozhrsf')
                console.log(new Array(depth).fill(0).reduce((av, cv) => { return av + '  '; }, ''), key, added, '(' + discWeight + ')');
            sums.push(added);
        });
    
    let result = total + sums.reduce((av, cv) => { return av + cv; }, 0);

    return result;
}

let mainKey = Object.keys(map)[0];
let root = map[mainKey];
let result = Object.keys(root)
    .map(key => {

        let tower = sum(root[key], 0, 1, key)
        let total = weights[key] + tower;

        return {
            key: key, 
            weight: total,
            discWeight: weights[key],
            towerWeight: total - weights[key]
        }
    });
